...
/*
=================
findspecificradius - rkh homing mod
- pass in userid and make that the target entity for homing
=================
*/
...
//		if (!from->inuse)
//			continue;
		if (from[i]->classname == "plasma")
			trap_SendServerCommand( -1, va("print \"found a plasma\""));
//		for (j=0; j<3; j++)
//			eorg[j] = org[j] - (from->r.currentOrigin[j] +(from->r.mins[j] + from->r.maxs[j])*0.5);
//		if (VectorLength(eorg) > rad)
//			continue;
