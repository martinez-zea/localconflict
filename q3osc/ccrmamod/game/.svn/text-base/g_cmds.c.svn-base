...
void Cmd_SetHoming_f (gentity_t *ent)
{
	...
	{
	trap_SendServerCommand( ent-g_entities, va("print \"Homing is off.\n\""));
	ent->client->pers.homing_status = 0;
	g_homing_status.integer = 0;
	}
	else
	{
	trap_SendServerCommand( ent-g_entities, va("print \"Homing is on.\n\""));
	ent->client->pers.homing_status = 1;
	g_homing_status.integer = 1;
	}
}
...
void ClientCommand( int clientNum ) {
	...
	if (Q_stricmp (cmd, "give") == 0)
		...
		Cmd_SetPlasmaBounce_f( ent );		//rkh
	else if (Q_stricmp (cmd, "bfgbounce") == 0)	//rkh - bounce mod
		Cmd_SetBFGBounce_f( ent );		//rkh
	else if (Q_stricmp (cmd, "parenthoming") == 0)	//rkh - bounce mod
		Cmd_SetParentHoming_f( ent );		//rkh
	else if (Q_stricmp (cmd, "parenthomingonly") == 0)	//rkh - bounce mod
		Cmd_SetParentHomingOnly_f( ent );	
	else if (Q_stricmp (cmd, "plasmapersist") == 0)
		Cmd_SetPlasmaPersist_f( ent);
	else if (Q_stricmp (cmd, "bfgpersist") == 0)
		Cmd_SetBFGPersist_f( ent);
	else if (Q_stricmp (cmd, "plasmahomingpersist") == 0)
		Cmd_SetPlasmaHomingPersist_f( ent);
	else if (Q_stricmp (cmd, "bfghomingpersist") == 0)
