...
//Q3apd puts its fudi build_socket, send methods here
/* ==============================================================
RKH - osc
=================================================================*/
//#ifndef Q3_VM
...
/*void osc_sockerror(char *s) {
  fprintf(stderr, "%s: %s (%d)\n", s, strerror(errno), errno);
}*/

/*int osc_build_socket() {

  struct hostent *hp;
  struct sockaddr_in server;

  fudi_sockfd = socket(AF_INET, SOCK_DGRAM, 0);
  if (fudi_sockfd < 0)
    {
    	fudi_sockerror("error calling socket()");
    	return 0;
    }

  hp = gethostbyname(fudi_hostname.string);

  if (hp == 0)
    {
      fudi_sockerror("unknown host");
      close(fudi_sockfd);
      fudi_sockfd = -1;
      return 0;
    }
...
/*void osc_send_real(char *msg) {
  ...
  nsend = strlen(msg);
  for (bp = msg, nsent = 0; nsent < nsend;)
    {
      int res = send(fudi_sockfd, msg, nsend-nsent, 0);
      if (res < 0)
	{
	  fudi_sockerror("error calling send");
	  close(fudi_sockfd);
	  fudi_sockfd = -1;
	  return;
	}
...
/*void osc_send(char *msg) {
  ...
  if (fudi_sockfd>=0) {
    ...
    //are the host/port cvars new?

    if (fudi_hostname.modificationCount>fudi_hostname_last_mod
	|| fudi_port.modificationCount>fudi_port_last_mod) {
      // yes

      fudi_hostname_last_mod = fudi_hostname.modificationCount;
      fudi_port_last_mod = fudi_port.modificationCount;
...
/*void osc_send_int(int clientno,char *label,int val) {
  char buf[1024];
  snprintf(buf,1024,"%s %d %d\n",label,clientno,val);
  osc_send(buf);
}*/
//#endif
/* =================================================================
...
/*
	...
	osc_client_vars currentClient;
	currentClient.clientnum = pm.ps->clientNum;
	currentClient.port = osc_client_port.string;
	currentClient.origin[0] = pm.ps->origin[0];
	currentClient.origin[1] = pm.ps->origin[1];
	currentClient.origin[2] = pm.ps->origin[2];
	...
	if (osc_send_client.integer == 1)
	{
		if(slork_switch.integer == 0){
			...
			if(osc_bundle.integer == 1)
		     ...
		     switch (currentClient.clientnum) {
			case 0:
				currentClient.hostname = osc_hostname1.string;
				break;
			case 1:
			...
			case 2:
				...
				break;
			case 3:
				currentClient.hostname = osc_hostname4.string;
				break;
			case 4:
				currentClient.hostname = osc_hostname5.string;
				break;
			case 5:
				currentClient.hostname = osc_hostname6.string;
				break;
			case 6:
				currentClient.hostname = osc_hostname7.string;
				break;
			case 7:
				currentClient.hostname = osc_hostname8.string;
				break;
			case 8:
				currentClient.hostname = osc_hostname9.string;
				break;
			case 9:
				currentClient.hostname = osc_hostname10.string;
				break;
			case 10:
				currentClient.hostname = osc_hostname11.string;
				break;
			case 11:
			...
			case 12:
				...
				break;
			case 13:
				currentClient.hostname = osc_hostname14.string;
				break;
			case 14:
				currentClient.hostname = osc_hostname15.string;
				break;
			case 15:
				currentClient.hostname = osc_hostname16.string;
				break;
			case 16:
				currentClient.hostname = osc_hostname17.string;
				break;
			case 17:
				currentClient.hostname = osc_hostname18.string;
				break;
			case 18:
				currentClient.hostname = osc_hostname19.string;
				break;
			case 19:
				currentClient.hostname = osc_hostname20.string;
				break;
			default:
				currentClient.hostname = osc_client_hostname.string;
				break;
...
//	trap_SendServerCommand( -1, va("print\"%s %s\"", osc_hostname.string, osc_port.string ));
//	if(strcmp(tempHostname, currentClient.hostname)!=0||strcmp(tempPort, currentClient.port)!=0 ){
//		trap_SendServerCommand( -1, va("print\"%s %s\"", currentClient.hostname, currentClient.port ));
...
/*
		...
		if (osc_open.integer) {
		  ...
		  osc_send_cached_int(clientno,pm.ps->pm_flags,pm_flags,"pm_flags");

		  if ((level.time-osc_cache[clientno].last_send)>=osc_rate.integer) {
		    snprintf(buf,1024,"stream %d %f %f %f %f %f %f %f %f %f %d %d %d\n",pm.ps->clientNum,
		  ...
		  osc_send_cached_int(clientno,pm.ps->weapon,weapon,"weapon");
		  osc_send_cached_int(clientno,pm.ps->weaponstate,weaponstate,"weaponstate");

		  osc_send_cached_int(clientno,pm.ps->jumppad_ent,jumppad_ent,"jumppad_ent");
		  osc_send_cached_int(clientno,pm.ps->damageEvent,damageEvent,"damageEvent");
		  osc_send_cached_int(clientno,pm.ps->damageYaw,damageYaw,"damageYaw");
		  osc_send_cached_int(clientno,pm.ps->damagePitch,damagePitch,"damagePitch");
		  osc_send_cached_int(clientno,pm.ps->damageCount,damageCount,"damageCount");
		  osc_send_cached_int(clientno,pm.ps->groundEntityNum,groundEntityNum,"groundEntityNum");
		  osc_send_cached_int(clientno,pm.surfaceFlags,surfaceFlags,"surfaceFlags");
		}
end rkh*/
//#endif
